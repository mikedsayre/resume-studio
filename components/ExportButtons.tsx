...
const ExportButtons: React.FC<ExportButtonsProps> = ({ onExportPdf, onCopyMarkdown, onCopyHtml }) => {
  ...
  return (
    <div className="flex items-center w-full sm:w-auto gap-x-3 gap-y-2"> {/* Apply gap here for all items */}
      <InfoIcon
        id="export-buttons-info"
        tooltipText="Manage your resume: Export as PDF, copy Markdown or HTML. Use custom CSS for advanced styling; for full control, especially over font styles, you may need to use !important in the custom CSS editor."
        tooltipAlign="right" // Align tooltip to the right, causing it to expand leftwards
        tooltipPlacement="bottom" // Changed to bottom to avoid header overlap
        // Removed className="ml-auto" as it's no longer needed for right alignment
      />
